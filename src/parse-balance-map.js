...
var isAddress = ethers_1.utils.isAddress,
...
function parseBalanceMap(balances) {
  ...
  var dataByAddress = balancesInNewFormat.reduce(function (memo, _a) {
    var account = _a.address,
      earnings = _a.earnings,
      reasons = _a.reasons;
  ...
  // construct a tree
  var tree = new balance_tree_1['default'](
  ...
  var claims = sortedAddresses.reduce(function (memo, address, index) {
    var _a = dataByAddress[address],
      amount = _a.amount,
      flags = _a.flags;
    memo[address] = __assign(
      { index: index, amount: amount.toHexString(), proof: tree.getProof(index, address, amount) },
      flags ? { flags: flags } : {}
    );
    return memo;
  }, {});
  var tokenTotal = sortedAddresses.reduce(function (memo, key) {
